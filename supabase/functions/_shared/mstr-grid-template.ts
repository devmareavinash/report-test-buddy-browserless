...
export function parseGridColumns(scenario: any, existingScript?: any): string[] {
  const out: string[] = [];
  ...
  if ((/geography details/i.test(blob) || /performance\s*trends?/i.test(blob)) && out.length < 8) {
...
export function assembleGridScript(opts: {
  ...
  const colsLit = JSON.stringify(expected.length ? expected : DEFAULT_GEOGRAPHY_COLUMNS, null, 4).replace(/\n/g, "\n    ");
